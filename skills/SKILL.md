...
- **Run tags**: Runs can carry multiple tags. Use `smoke` for smoke or structural checks and `malfunctioning` for malfunctioning runs. Metrics Explorer excludes `smoke` by default. Tags are shared definitions with editable UI colors; assignments are many-to-many. SDK smoke checks can call `Run(..., tags=["smoke"])`, `run.add_tag("smoke")`, or `ClausiusSession.start_from_cli(..., tags=["smoke"])`.
