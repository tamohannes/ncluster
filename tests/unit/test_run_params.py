...
def test_sanitize_params_preserves_nested_dicts_up_to_max_depth():
    sanitize = _load_sanitize_params()
    ...
    cursor = out
    for _ in range(5):
        assert isinstance(cursor, dict), f"depth reached non-dict early: {cursor!r}"
        cursor = cursor["k"]
    assert cursor == leaf


def test_sanitize_params_stringifies_beyond_max_depth():
    sanitize = _load_sanitize_params()

    deep = {"k": {"k": {"k": {"k": {"k": {"k": "buried"}}}}}}
    out = sanitize(deep)
    cursor = out
    for _ in range(5):
        cursor = cursor["k"]
    assert isinstance(cursor, str)
    assert "buried" in cursor
